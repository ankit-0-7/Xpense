...
// --- IMPORT PATHS ---
// Ensure these images are inside client/src/
      ...
      <>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
          <motion.div
            ...
            className="bg-[#18181b] w-full max-w-md rounded-2xl border border-white/10 shadow-2xl overflow-hidden relative"
          >
...
const LandingPage = ({ onLoginSuccess }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      onLoginSuccess(); // Notify App.jsx to switch to Dashboard
    } catch (err) {
    ...
    <div className="min-h-screen bg-black text-white font-sans overflow-hidden relative">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        ...
        <img 
            ...
            className={`w-full h-full object-cover scale-105 transition-opacity duration-1000 ease-out ${imgLoaded ? 'opacity-100' : 'opacity-0'}`}
        />
      ...
      <nav className="flex justify-between items-center p-6 max-w-7xl mx-auto relative z-10">
        <div className="flex items-center gap-2">
          <img src={logo} alt="Xpense Logo" className="h-16 w-auto object-contain" />
        </div>
      ...
      <main className="max-w-5xl mx-auto px-6 flex flex-col items-center justify-center min-h-[69vh] relative z-10 text-center">
        ...
        <div className="mt-10 lg:mt-0 flex flex-col items-center">
          <motion.div 
            ...
            className="text-5xl lg:text-7xl font-extrabold leading-tight tracking-tight flex flex-col items-center gap-2 drop-shadow-2xl"
          >
              ...
              <div className="relative h-[1.1em] w-[4.5em] lg:w-[5em] text-right">
                <AnimatePresence mode="wait">
                  <motion.span
                    ...
                    className="absolute right-0 bg-gradient-to-r from-emerald-400 to-purple-500 bg-clip-text text-transparent"
                  >
                    {words[index]}
                  </motion.span>
                </AnimatePresence>
          ...
          <motion.p 
            ...
            className="text-gray-200 text-lg lg:text-xl mt-8 max-w-2xl mx-auto font-medium drop-shadow-md"
          >
            Track expenses, manage budgets, and get AI-powered insights. 
            Stop worrying about where your money goes and start building your future.
          </motion.p>
          ...
          >
            <button 
              ...
              className="px-8 py-3.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-white font-bold text-lg transition-all shadow-[0_0_20px_rgba(16,185,129,0.4)] hover:shadow-[0_0_30px_rgba(16,185,129,0.6)]"
            >
              Login
            </button>
            <button 
              onClick={() => setShowRegister(true)}
              className="px-8 py-3.5 rounded-xl bg-[#2e1065] hover:bg-[#3b0764] border border-purple-500/30 text-purple-300 hover:text-white font-bold text-lg transition-all backdrop-blur-md"
            >
              Register
            </button>
          </motion.div>
      ...
      <Modal isOpen={showLogin} onClose={() => setShowLogin(false)} title="Welcome Back">
        <form onSubmit={handleLogin} className="space-y-4">
          <InputField 
            icon={<Mail size={20} />} type="email" placeholder="Email Address" 
            value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})}
          />
          <InputField 
            icon={<Lock size={20} />} type="password" placeholder="Password" 
        ...
        <p className="mt-6 text-center text-gray-400">
          Don't have an account? <span onClick={() => {setShowLogin(false); setShowRegister(true)}} className="text-emerald-400 font-bold cursor-pointer hover:underline">Register</span>
        </p>
      </Modal>
      ...
      <Modal isOpen={showRegister} onClose={() => setShowRegister(false)} title="Create Account">
        <form onSubmit={handleRegister} className="space-y-4">
          <InputField 
            icon={<User size={20} />} type="text" placeholder="Full Name" 
        ...
        <p className="mt-6 text-center text-gray-400">
          Already have an account? <span onClick={() => {setShowRegister(false); setShowLogin(true)}} className="text-emerald-400 font-bold cursor-pointer hover:underline">Login</span>
        </p>
      </Modal>
